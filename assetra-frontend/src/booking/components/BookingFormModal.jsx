  ...
  return (
    <input
      className={`w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600
        ...
        focus:border-amber-400 transition-all ${className}`}
      {...props}
    />
  );
}
...
export default function BookingFormModal({ resource, onClose, onSuccess }) {
  ...
  const validate = () => {
    ...
    if (resource.capacity && form.expectedAttendees &&
        Number(form.expectedAttendees) > resource.capacity)
      e.expectedAttendees = `Cannot exceed capacity of ${resource.capacity}`;
    return e;
  };
        ...
        {success ? (
          ...
          <>
            <div className="p-6 space-y-4">
              ...
              {resource.capacity && (
                <Field label={`Expected Attendees (max ${resource.capacity})`} error={errors.expectedAttendees}>
                  <Input type="number" min="1" max={resource.capacity}
                    placeholder={`Up to ${resource.capacity}`}
