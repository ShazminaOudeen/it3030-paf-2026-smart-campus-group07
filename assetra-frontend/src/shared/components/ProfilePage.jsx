...
import {
  User, Mail, Phone, Shield, Check, Calendar,
  Pencil, X, Save, Sparkles, Award, Clock,
...
function useRoleAccent() {
  ...
  if (pathname.startsWith("/admin"))
    return {
      glow: "rgba(249,115,22,0.15)", dot: "#f97316",
      tag: "Administrator", portal: "Admin Portal",
    };
  if (pathname.startsWith("/technician"))
    return {
      glow: "rgba(59,130,246,0.15)", dot: "#3b82f6",
      tag: "Technician", portal: "Technician Portal",
    };
  return {
    glow: "rgba(245,158,11,0.12)", dot: "#f59e0b",
    tag: "Campus User", portal: "User Portal",
  };
}
...
function Field({ label, value, name, type = "text", editing, onChange, icon: Icon, locked, error }) {
  return (
    <div className="flex flex-col gap-1.5 group transition-all duration-200">
      <label className="flex items-center gap-1.5 text-[10px] font-semibold tracking-widest uppercase
        text-gray-400 dark:text-gray-500 transition-colors">
        <Icon size={10} />
        {label}
        {locked && editing && (
          <span className="ml-auto text-[9px] normal-case tracking-normal font-normal text-gray-400 dark:text-gray-600">
            locked
          </span>
        )}
      </label>
      ...
      {editing && !locked ? (
        <>
          <input
            ...
            className={`w-full rounded-xl px-4 py-2.5 text-sm text-gray-900 dark:text-white
              bg-gray-50 dark:bg-white/[0.05] border outline-none transition-all duration-200
              placeholder-gray-400 focus:scale-[1.01] focus:shadow-lg
              ${error
        ...
        <div className="px-4 py-2.5 rounded-xl text-sm text-gray-700 dark:text-gray-300
          bg-gray-50/80 dark:bg-white/[0.03] border border-gray-100 dark:border-white/[0.05]
          min-h-[42px] flex items-center transition-all duration-200
          group-hover:border-gray-200 dark:group-hover:border-white/[0.1]">
  ...
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
      <div className="absolute inset-0" style={{
        background: `radial-gradient(circle at 70% 30%, ${accent.glow} 0%, transparent 70%)`,
      }}/>
      <div className="absolute inset-0" style={{
...
export default function ProfilePage() {
  ...
  /* ✅ FIXED handleSave — correctly updates DB, localStorage, and AuthContext */
  const handleSave = async () => {
    ...
    try {
      ...
      // Build updated user — use DB response if available, fall back to form values
      const updatedUser = { ...user };
      if (res.ok) {
        const data = await res.json();
        updatedUser.name  = data.name  || form.name;
        updatedUser.phone = data.phone || form.phone;
      } else {
      ...
      // ✅ Update AuthContext state correctly
    ...
    <>
      <style>{`
        ...
        .animate-fade-slide-right {
          animation: fadeSlideRight 0.5s cubic-bezier(0.4,0,0.2,1) both;
        }
        .animate-badge-pulse {
      ...
      <div className="max-w-2xl mx-auto space-y-4 pb-12 px-4 sm:px-6">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] font-semibold tracking-widest uppercase px-2.5 py-0.5 rounded-full
                    ...
                    text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-white/[0.08]
                ...
                <p className="text-[11px] text-gray-400 dark:text-gray-500 font-mono mt-0.5 flex items-center gap-1">
                  <Clock size={10}/> {accent.portal} · Member since {memberSince}
                </p>
              </div>
                  ...
                  <>
                    <button onClick={handleCancel}
                      className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold
                        text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-white/[0.06]
                        hover:bg-gray-200 dark:hover:bg-white/10 border border-gray-200 dark:border-white/[0.08]
                        hover:scale-105 transition-all duration-200">
                      <X size={12}/> Cancel
                    </button>
                    <button onClick={handleSave} disabled={saving}
                      className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold
                        text-white bg-gradient-to-r from-gray-800 to-gray-900
                        dark:from-white/90 dark:to-white/80 dark:text-gray-900
        ...
        <div className="profile-card rounded-3xl bg-white dark:bg-gray-900/95
          ...
          <div className="flex items-center justify-between">
            ...
            {editing && (
              <p className="text-xs text-gray-400 dark:text-gray-500 flex items-center gap-1">
                <ShieldCheck size={10}/> Email cannot be changed
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field
              ...
              editing={editing} onChange={handleChange} icon={User} error={errors.name}/>
            <Field
              label="Email Address" name="email" value={form.email} type="email"
              editing={editing} onChange={handleChange} icon={Mail} locked error={errors.email}/>
            <Field
              label="Contact Number" name="phone" value={form.phone} type="tel"
              editing={editing} onChange={handleChange} icon={Phone} error={errors.phone}/>
            <Field
          ...
          {editing && (
            <div className="mt-2 p-3 rounded-xl
              bg-gradient-to-r from-gray-50 to-gray-100 dark:from-white/[0.03] dark:to-white/[0.01]
              border border-gray-100 dark:border-white/[0.05]">
              <p className="text-xs text-gray-400 dark:text-gray-500 space-y-1">
                <span className="block">• Name: letters and spaces only, no numbers or special characters</span>
                <span className="block">• Phone: must start with 07 and be exactly 10 digits (e.g. 0771234567)</span>
              </p>
            </div>
          )}
        </div>
            ...
            {[
              ...
              <div key={label}
                className={`flex items-center justify-between py-3 group
                  hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors duration-200
        ...
        <p className="text-center text-[10px] text-gray-300 dark:text-gray-700 font-mono pt-2">
